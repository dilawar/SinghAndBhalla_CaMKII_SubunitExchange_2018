...
def ca_input( root ):
    ...
    _logger.debug( "Ca conc expression = %s " % concFunc.expr )
...
def pp1_activation( root ):
    ...
    I1P binds with PP1 and give rise to complex PP1_ (which is inavtivated PP1)
    I1P + PP1 <- -> PP1_
    """
    inactive_pp1 = moose.Pool( '%s/PP1_' % root.path )
    inactive_pp1.name = pool_name( root, 'PP1_' )
    molecules_[inactive_pp1.name] = inactive_pp1
    ...
    moose.connect( r, 'sub', i1pPool, 'reac' )
...
def dephosphorylate( frmPool, toPool, michealson_mentis = False ):
    ...
    if michealson_mentis:
        ...
        moose.connect( rr, 'prd', molecules_[curr_subsec_ + '.' + 'PP1_'], 'reac' )
...
def camkii_activation_deactivation( root ):
    ...
    for nP in range(1, 7):
        ...
        dephosphorylate( frmPool, toPool )
...
def make_model( root, **kwargs):
    ...
    if args['enable_subunit_exchange']:
        ...
        if False:
            #  dephosphorylate x to y. A subunit can also be dephosphorylated by PP1.
            dephosphorylate( molecules_[ pool_name( root, 'x') ], molecules_[ pool_name( root, 'y' ) ] )
        else:
    ...
    add_turnover( root )
    add_x_diffusion_approximation( root
            , kf = args.get('xtoxsKf', 0.0) 
            , kb = args.get('xtoxsKb', 0.0) 
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-voxels', '-nv'
        ...
        , type = int
        , help = 'No of voxels in one switch. Each voxel must be bistable.'
        )
    ...
    parser.add_argument('--xtoxsKf'
        , required = False
        , default = 0.0
        , type = float
        , help = 'Mimic x diffusion. Forward rate. See labnote.md'
        )

    parser.add_argument('--xtoxsKb'
        , required = False
        , default = 0.0
        , type = float
        , help = 'Mimic x diffusion. Backward rate. See the labnote'
        )
