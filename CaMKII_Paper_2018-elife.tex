...
This would require a detailed 3D simulation and is beyond the scope of this study.

Finally, we suggest that existence of diverse multiple time-scales of
...
dependent changes (learning) but bad at retaining old memories. On the other
...
these two competing demands requires that such diversity of timescales must
...
with number of synapses, and memory decay follows \(1/\sqrt{t}\) --- a power-law
supported by psychological studies \citep{wixted_form_1991}. This model requires
memory trace to be first stored in a fast variable and then progressively and
efficiently transferred to slower variables.  Our study suggests a concrete
mechanism for such a process. Here, \gls{ca} concentration in PSD can be mapped
to the fastest variable. The \gls{camkii} integrator in cytosol could represent
