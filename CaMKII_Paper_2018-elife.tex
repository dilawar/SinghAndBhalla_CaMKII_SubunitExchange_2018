...
is the fluctuation in basal calcium levels, see \FIG{validation}C), and without subunit
...
bistable for \SUB{D}{PP1} $\gt$ \SI{0.5}{\micro\meter\squared\per\second}
(\FIG{method_validation}C). For very high values of \SUB{D}{PP1}, the system
diverged from well-mixed system again because potency of \gls{pp1} reduced with
increased \SUB{D}{PP1}
...
activity. This effect is not entirely due to numerical errors as shown in
\FIGSUPP[method_validation]{diffusion_reduces_pp1_potency}. 
...
\begin{figure}[ht!]
    ...
    \caption{Method validation. \SUB{N}{CaMKII}=6 holoenzymes as described in
        ...
        was fixed to \SI{100}{\nano M}+$\epsilon$.  
        %
        \textbf{(A, above)} For a typical value of \SUB{D}{PP1}=\SI{0.5}{\micro\meter\squared\per\second} 
        used in our model, varying \SUB{D}{sub} did not result in lost of
        bistability of \gls{camkii} activity.
        \textbf{(A, below)} Reference well-mixed system for comparison. 6 holoenzyme 
        were simulated in a single well-mixed cylinder of same length 
        and volume, and with same parameter values as above.
        %
        \textbf{(B)} Cumulative histograms of \gls{camkii} activity for
        various of values of \SUB{D}{PP1} and \SUB{D}{sub}
        (unit \si{\micro\meter\squared\per\second}). The red and blue lines
        represent spatially discretized and well-mixed reference system (shown
        in A, black) respectively. Spatially discretized system (red) converges to
        well-mixed system (blue) for higher \SUB{D}{PP1} values.
        For fixed value of \SUB{D}{PP1}, changing \SUB{D}{sub} has little or no
        effect on convergence. The convergence is sensitive to \SUB{D}{PP1}
        because \gls{pp1} is involved in bimolecular reactions and bimolecular
        reaction does not converge to well-mixed case
        \citep{isaacson_reaction-diffusion_2009}. For a typical value of
        ...
        similarity between two histogram. Two identical histograms will have
        zero Kullback-Leibler divergence. Phase plot shows Kullback-Leibler
        divergence of two histograms -- the spatially extended system and the
        reference bistable system. Black dots represent bistable configurations
        with at least 4 transitions observed in a simulation lasting 20 days. 
        Thus the spatially extended, discretized system converged to the behaviour
        of the reference bistable system.
    }\label{fig:method_validation} 

    % suppl
    \figsupp[\gls{pp1} potency reduces as \SUB{D}{PP1} increases.]{CaMKII/PP1
        system was simulated in a cylindrical arena discretized into 6 voxels 
        of equal volume separated by distance \(h\)=\SI{30}{\nano\meter}.
        %
        \textbf{(A)} \textbf{(above)} For each voxel, the trajectory of active \gls{pp1} vs. time
        ...
        of subunit (\SUB{D}{sub}). On the y-axis, \gls{pp1} activity is measured as
...
\begin{center}
    ...
    \captionof{figure}{Error estimate for the bimolecular reaction
        ...
        4 molecules of A and B each  were simulated in a cylindrical arena 
