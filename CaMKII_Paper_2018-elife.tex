...
(\FIG{tolerance_pp1}B, red curve). This served as the baseline to quantify the
effect of subunit exchange. When we enabled subunit exchange by setting
\SUB{D}{sub}=\SI{0.1}{\micro\meter\squared\per\second}, \gls{camkii} activity
rose to maximum within \SI{4}{\hour} (\FIG{subunit_facilitates_spread}B).
Similarly, raising basal \gls{ca} level to (\SI{120}{\nano M}) also had similar
effect. The system showed moderate \gls{camkii} activity with 
same values of parameters and the effect of subunit
