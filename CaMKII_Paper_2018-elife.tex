...
\FIG{subunit_facilitates_spread}B), \gls{camkii} activity rose to maximum within
\SI{4}{\hour}. As expected, the effect of subunit exchange (rise time quantified
as the time taken by \gls{camkii} to rise from 10\% to 90\%) was stronger when
the basal \gls{ca} was higher (\FIG{subunit_facilitates_spread}C). Increasing
\SUB{D}{sub} decreased the rise time of \gls{camkii} activity.
...
exchange) but was strongly dependent on \SUB{D}{PP1}. This is because potency of
\gls{pp1} reduced with increased \SUB{D}{PP1}
(\FIGSUPP[subunit_facilitates_spread]{diffusion_reduces_pp1_potency}) which led
to decreased \gls{pp1} activity and hence \gls{camkii} activity.
...
\begin{figure}
    ...
    \gls{ca} concentration of \SI{100}{\nano M}.]{In all simulations,
        \SUB{D}{PP1} is fixed to \SI{0.1}{\micro\meter\squared\per\second} and \SUB{D}{sub} was
...
\subsection{Subunit exchange synchronizes switching activity of clustered \gls{camkii}}
...
\gls{camkii} subunits (\SUB{D}{sub}=\SI{1}{\micro\meter\squared\per\second}) and
\gls{pp1} (\SUB{D}{PP1}=\SI{0.5}{\micro\meter\squared\per\second}). These
conditions decreased the rate of dephosphorylation of \gls{camkii} holoenzymes
roughly by a factor of 5 (\SI{41.65}{\second} v/s \SI{200.82}{\second})
...
\begin{figure}%[t]%[tbh]
    ...
    \caption{ In \gls{pp1} rich spine cytosol, \gls{camkii} acts as a leaky integrator of \gls{ca} 
        ...
        can explain observed two time-constants of \gls{camkii} decay \cite{chang_camkii_2017}.
        ...
        After the pulse, \gls{ca} levels were brought down to basal level
        \FIG{validation}C. Three trajectories are shown: without subunit exchange (red), with
        ...
        For a given $a=0.74$ (estimated in \cite{chang_camkii_2017}), our time-constants
...
they will exhibit long and short time-courses of activity decay and therefore a
mixed population of clustered and non-clustered \gls{camkii} will decay with two
time-constants.

The decay kinetics of \gls{camkii} obtained by curve fitting of experimental
data is given by a double-exponential function:
$F(t)=P_{fast}e^{-t/τ_{fast}}+P_{slow}e^{−t/\tau_{slow}}$ where $P_{fast}=0.74,
P_{slow}=0.26,  \tau_{fast}= 6.4\pm0.7 \si{\second}, \tau_{slow}=92.6 \pm
50.7\si{\second}$ \cite{chang_camkii_2017} (\FIG{cytosol_integrator}C, magenta).
We used this estimate of $P_{fast}$ and $P_{slow}$ to construct a mixed
population of slow and fast decaying \gls{camkii} (\FIG{cytosol_integrator}A,
black) and estimated the double-exponential function which fits it well
