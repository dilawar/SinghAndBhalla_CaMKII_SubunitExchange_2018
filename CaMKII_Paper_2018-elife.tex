...
bistable switch (\FIG{validation}C, D), 2. The stability of the switch
...
mass-action kinetics used in our mode (\EQ{dephosphorylation}). It required on
...
in \FIG{tolerance_pp1}B) and also became independent of \SUB{N}{PP1}.  Moreover,
...
constant upto a $\sim$10 fold increase in turnover (\FIG{turnover}B), after
...
literature that we are aware of. We used data available in
\citep{stratton_activation-triggered_2014} to estimate the timescale of subunit
exchange rate (see \ref{sec:estimate_exchange_rate}). Bhattacharya \textit{et
al}.  \citep{bhattacharyya_molecular_2016} speculate that upon activation, the
hub of holoenzyme becomes less stable and more likely to open up and lose a
subunit i.e., active holoenzymes may lose subunits faster. Therefore, we
maintained the following ratio \(k_x^- =\sim 10 k_x^+ N_{\text{CaMKII}}\) and
\(k_y^- =\sim 10 k_y^+ N_{\text{CaMKII}}\) in all simulations where \SUB{N}{CaMKII}
is the number of holoenzymes in the system.  

% Estimation of subunit exchange rate.
\subsubsection{Estimation of subunit exchange rate}\label{sec:estimate_exchange_rate}
...
were mixed together. Once a holoenzyme consisting of same color subunits picks
up a different color subunit, it emits both color lights and can be quantified
as such. The readout from this assay is ``cololocalization'' which is the
fraction of holoenzymes which have both colors subunits.
...
\begin{equation}\label{eq:mixing_assay}
    \begin{gathered}
        \schemestart $R_aG_b$ \arrow{<=>[$r_l$][$r_g$]} $R_{a-1}G_b$ + $R$ \schemestop
        \quad \text{for all}\; a>0,b\ge 0\; \text{s.t.}\; a+b=7 \\
        \schemestart $R_aG_b$ \arrow{<=>[$r_l$][$r_g$]} $R_{a}G_{b-1}$ + $G$ \schemestop
        \quad \text{for all}\; a\ge 0,b> 0\; \text{s.t.}\; a+b=7
    \end{gathered}
\end{equation}

To compute the value of colocalization, we compute the percentage of holoenzymes
with at least one red and one green subunit i.e. $\frac{\sum_{a\ge 1, b \ge 1}
[R_aG_b]}{ \sum_{a\ge0,b \ge 0}[R_aG_b]}$. We simulate this reaction network to
estimate values of rate constants $r_g$ and $r_l$ which best describe the data
presented in \cite{stratton_activation-triggered_2014} at various \gls{camkii}
concentration (\FIG{estimate_of_exchange_rate}). Our model captures the dynamics
of colocalization (\FIG{estimate_of_exchange_rate}C) and concentration dependent
reduction in colocalization very well (\FIG{estimate_of_exchange_rate}D). The
values of 

\begin{figure}
\includegraphics[]{PaperFigures/suppl/ExperimentalDataStrattonELife2016/figure_exchange_rate.pdf}
\captionof{figure}{Estimate of subunit exchange rate. 
