...
\subsection{Subunit exchange facilitates the spread of \gls{camkii} activity}
...
conditions decreased the rate of dephosphorylation roughly by a factor of 5 
...
\begin{figure}%[t]%[tbh]
    ...
    %% Change Dsub and then plot it.
    %%\figsupp[\gls{camkii} activity decays slower with decreasing voxel size.]{ 
    %%    \gls{camkii} activity decays slower with
    %%    decreasing inter \gls{camkii} distance.  Simulation arena (i.e. cylinder) 
    %%    is divided into 20 voxels as compared to 10 in \FIG{cytosol_integrator} (5 \gls{camkii}
    %%    in each voxels v/s 10 \gls{camkii} in each voxel). See
    %%    \FIG{cytosol_integrator} for panel description.
    %%}{\includegraphics[]{./PaperFigures/suppl/figure_two_timecourses_114cm_sup.pdf}}\label{figsupp:slower_decay_tighter_clustering}
\figdata{Source and data are available at
