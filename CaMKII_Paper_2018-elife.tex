...
\begin{figure}[ht!]
    ...
    \caption{Method validation. \SUB{N}{CaMKII}=6 holoenzymes as described in
        ...
        with volume \SI{1e-21}{\cubic\meter}, divided into 3 equal voxels separated by 
        \SI{30}{\nano\meter}. Basal \gls{ca} was fixed to \SI{100}{\nano M}+$\epsilon$.  
        %
        \textbf{(A, above)} For fixed 
        \SUB{D}{PP1}=\SI{0.1}{\micro\meter\squared\per\second}, varying
        \SUB{D}{sub} did not affect average \gls{camkii} activity. 
        \textbf{(A, below)} Reference system for comparison. 6 holoenzyme 
        were simulated in a single well-mixed cylinder of same volume, and 
        with same parameter values as above.
        %
        \textbf{(B)} On the other hand, for fixed
        \SUB{D}{sub}=\SI{0.5}{\micro\meter\squared\per\second} , increased
        \SUB{D}{PP1}, increased the average \gls{camkii} activity (red and
        black). We obtained bistable trajectories (blue) which were switching 
        between high and low levels of active CaMKII) which was similar to 
        the behaviour of the reference bistable system for
        \SUB{D}{PP1}=\SI{0.1}{\micro\meter\squared\per\second}.
        %
        ...
        Black dots represent bistable configurations (histogram of these
        trajectories has two distinguishable peaks). Thus the spatially extended, 
        discretized system converged to the behaviour of the reference bistable
        system (A, below). This convergence is lost for smaller volumes due to
        increased stochastic effect e.g. when cylinder was
        divided into 6 or 12 equal voxels (data not shown).
