...
In this experiment, we release NCaMKII/2 active subunits and all
CaMKII is symm 6.
...
def conc_to_n( conc ):
    assert args['voxel_volume'] > 0.0, "Got %g" % args['voxel_volume']
    val = conc * args['voxel_volume'] * N
    assert val > 0.0, 'Computed %g for conc %g' % (val, conc)
    return val

def compartment_to_graph( root = '' ):
    g = nx.MultiDiGraph( )
    reacs = moose.wildcardFind( '%s/##[TYPE=Reac]' % root )
    reacs += moose.wildcardFind( '%s/##[TYPE=ZombieReac]' % root )
    for r in reacs:
        subs, prds = r.neighbors[ 'sub' ], r.neighbors[ 'prd' ]
        for p in prds:
            print( '%s --> %s' % (p.name, ', '.join([x.name for x in subs ])))
            for s in subs:
                g.add_edge( p.name, s.name )

    nx.drawing.nx_agraph.write_dot( g, '%s.dot' % sys.argv[0] )
    print( 'Wrote network topology' )
...
def inv_conc_to_N( conc ):
    return conc / (float(args['voxel_volume']) * N )

def n_to_conc( n ):
    global args
    return n / (N * args['voxel_volume'] )
...
def dephosphorylate( root, frmPool, toPool ):
    ...
    _logger.debug( 'Not using Michealson Menten scheme for dephosphorylate' )
    ...
    for r in reacs:
        ...
        for ss in r.neighbors['sub']:
            for s in ss:
                ...
                molecules.append( 
                    '"%s" [label="%s",shape=circle,diffConst="%g",init="%f"]; ' % (
                    s.path, "", s.diffConst, s.nInit ) 
                    )
        rtext += '\t' + ' , '.join( subs )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--voxel-length', '-vl'
        , required = False
